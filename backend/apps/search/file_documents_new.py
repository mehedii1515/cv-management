...
class FileDocument(Document):
    ...
    # File Metadata
    file_type_category = Keyword()             # resume, document, etc.
    ...
    def create_from_file(cls, file_path: str, base_directory: str = None):
        ...
        try:
            ...
            # Categorize file
            file_category = cls.categorize_content(content, filename)
    ...
    def detect_language(content: str) -> str:
        ...
        try:
            ...
            detected = detect(content[:1000])  # Use first 1000 chars for detection
    ...
    def categorize_content(content: str, filename: str) -> str:
        """Categorize file content based on keywords and filename"""
        content_lower = content.lower()
        filename_lower = filename.lower()
        
        # Resume indicators
        resume_keywords = [
            'curriculum vitae', 'resume', 'cv', 'experience', 'education',
            'skills', 'employment', 'qualifications', 'objective', 'summary',
            'work history', 'achievements', 'references', 'certifications'
        ]
        
        # Cover letter indicators
        cover_letter_keywords = [
            'dear', 'sincerely', 'cover letter', 'application', 'position',
            'apply', 'interested in', 'look forward', 'thank you for considering'
        ]
        
        # Portfolio indicators
        portfolio_keywords = [
            'portfolio', 'projects', 'gallery', 'showcase', 'samples',
            'work samples', 'case studies', 'design', 'artwork'
        ]
        ...
        if any(word in filename_lower for word in ['resume', 'cv']):
            ...
            return 'cover_letter'
        elif any(word in filename_lower for word in ['portfolio', 'project']):
            return 'portfolio'
        
        # Check content
        resume_score = sum(1 for keyword in resume_keywords if keyword in content_lower)
        cover_letter_score = sum(1 for keyword in cover_letter_keywords if keyword in content_lower)
        portfolio_score = sum(1 for keyword in portfolio_keywords if keyword in content_lower)
